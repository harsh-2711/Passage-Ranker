...
def IDF_Generator(corpusfile, delimiter=' ', base=math.e) :
    ...
    for line in open(corpusfile,"r",encoding="utf-8") :
        ...
        doc = list(set(tokens)) # Take all unique words
    ...
    for word in docFrequencyDict:  #Calculate IDF scores for each word(q_i)
        docIDFDict[word] = math.log((numOfDocuments - docFrequencyDict[word] + 500) / (docFrequencyDict[word] + 500), base) #Why are you considering "numOfDocuments - docFrequencyDict[word]" instead of just "numOfDocuments"
...
def GetBM25Score(Query, Passage, k1=1.5, b=0.75, delimiter=' ') :
    ...
    token1 = word_tokenize(Query)
    query_words = list(set(token1))
    ...
    token2 = word_tokenize(Passage)
    passage_words = list(set(token2))
    passage_words = [w for w in passage_words if not w in stop_words]
    passage_words = [porter.stem(word) for word in passage_words]
...
if __name__ == '__main__' :
    ...
    inputFileName = "tempDataForLocal.tsv"   # This file should be in the following format : queryid \t query \t passage \t label \t passageid
    testFileName = "validationForCorpusLocal.tsv"  # This file should be in the following format : queryid \t query \t passage \t passageid # order of the query
